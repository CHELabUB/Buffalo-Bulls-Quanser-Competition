...
class TrajectoryPlanner:
    ...
    def line_detect(self, image):
        ...
        # road color is darker, this aims to filter out the road area, then based on road area, detect the boundary line
        lower = np.array([0, 0, 0])
        upper = np.array([180, 255, 100])  # V < 100
        mask = cv2.inRange(hsv_rgb, lower, upper)
        h, w = mask.shape

        # ignore bottom 10% and right 2% in the detection mask only;
        # detected (u, v) still remain in original image coordinates
        cut_row_start = int((1.0 - self.cut_bottom_ratio) * h)
    ...
    def line_remap(self, x, z):
        u,v = self.c2p_ground(self, x, z, self.d435_height)
        return u, v
